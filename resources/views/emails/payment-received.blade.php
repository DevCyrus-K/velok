...
<tr><td style="padding:24px"><h1 style="margin:0 0 8px;color:#166534;font-size:22px">Payment received</h1>
<p>Hello {{ $invoice->customer_name }}, payment for invoice {{ $invoice->invoice_number }} has been recorded.</p>
<p><strong>Amount:</strong> KES {{ number_format((float) $invoice->total_amount, 2) }}<br><strong>Paid at:</strong> {{ $invoice->paid_at?->format('d M Y H:i') ?? now()->format('d M Y H:i') }}</p>
<p style="margin-bottom:0">Thank you.</p></td></tr>
</table>
</td></tr></table>
